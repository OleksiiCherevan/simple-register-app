...
const UserSignUp = (props) => {
    ...
    useEffect(()=> {
        let isCorrect = !!(name.isCorrect && email.isCorrect && phone.isCorrect);
        let result = getInfo(isCorrect)
        onChange(result)
    }, [name, email, phone])

    const getInfo = (isCorrect) => {
        return {
            name: name.text,
            email: email.text,
            phone: phone.text,
    ...
    return (
        <div className={style["sign-up"]}>
            <TextBox placeholder={'Your name'} errorMessage={'Incorrect name! Please use letters!'} pattern={NAME_PATTERN} onChange={setName}></TextBox>

            <TextBox placeholder={'Email'} errorMessage={'Incorrect email! Please use valid email!'} pattern={EMAIL_PATTERN} onChange={setEmail}></TextBox>

            <TextBox placeholder={'Phone'} errorMessage={'Incorrect phone! Please use format like (+380123456789)!'} pattern={PHONE_PATTERN} onChange={setPhone}></TextBox>
        </div>
    );
};
